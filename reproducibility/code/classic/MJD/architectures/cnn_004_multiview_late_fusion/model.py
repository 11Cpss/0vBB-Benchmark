"""Multi-view late-fusion CNN for MJD waveforms."""
...
class MultiViewLateFusionBackbone(nn.Module):
    """Encode three fixed STFT views independently with shared weights."""
...
class MultiViewLateFusionRegressor(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.backbone = MultiViewLateFusionBackbone(**MODEL_CONFIG)
        self.head = nn.Linear(self.backbone.output_features, 1)
...
def build_model(task: str) -> nn.Module:
    ...
    if task == "regression":
        return MultiViewLateFusionRegressor()
