...
[`energybench_workflow.ipynb`](energybench_workflow.ipynb) follows the same
workflow with the standardized configuration unchanged.

## 1. Install and start

Use Python 3.11 and install the PyTorch build appropriate for the machine. From
the standalone folder, install the remaining dependencies and start Jupyter:
