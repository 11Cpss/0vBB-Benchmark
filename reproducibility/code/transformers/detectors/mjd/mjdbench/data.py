...
class _FileInfo:
    ...
    physical_energies: np.ndarray | None
...
class MJDWaveformDataset(Dataset):
        ...
        for raw_path in paths:
            ...
            try:
                with h5py.File(path, "r") as handle:
                    ...
                    physical_energies: np.ndarray | None = None
    ...
    def _output(
        ...
        if self.task != "inference":
            ...
            if info.physical_energies is None:
                raise RuntimeError("raw physical MJD energy is unavailable")
            output["energy_keV"] = torch.tensor(
                float(info.physical_energies[row]), dtype=torch.float64
            )
