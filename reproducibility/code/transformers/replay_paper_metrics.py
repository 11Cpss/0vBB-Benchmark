#!/usr/bin/env python3
"""Reproduce the published NEXT and SuperNEMO Transformer metrics.

Native saved predictions are supplied separately through --data-root.
The corresponding paper profiles are implemented under benchmark/.
"""
...
def worker(dataset,manifest,data_root,output):
    path=ROOT/'benchmark/published_profiles'/('next_transformer' if dataset=='NEXT' else 'supernemo_transformer')
    sys.path.insert(0,str(ROOT))
    import numpy as np
    if dataset=='NEXT':
        from benchmark.published_profiles.next_transformer.energybench.metrics import evaluate_energy_matched_classification,evaluate_energy_dependence
    else:
        from benchmark.published_profiles.supernemo_transformer.energybench.roc import evaluate_energy_matched_roc
        from benchmark.published_profiles.supernemo_transformer.energybench.dependence import evaluate_dependence
    results=[]
    for r in json.loads(manifest.read_text())['records']:
        if r['dataset']!=dataset:continue
        p=data_root/r['relative_input'];assert sha(p)==r['sha256'],f'Input checksum differs: {p}'
        with np.load(p,allow_pickle=False) as z:a={k:z[k] for k in z.files}
        if dataset=='NEXT':
            cfg=dict(r['evaluation_config'],distance_correlation_max_samples=4)
            ...
            cfg=r['evaluation_config'];c=cfg['classification'];d=cfg['dependence']
        ...
        errors={k:abs(values[k]-r['expected_metrics'][k]) for k in values}
        assert max(errors.values())<1e-12,(dataset,r['model_key'],errors)
        result=dict(dataset=dataset,model_key=r['model_key'],n_events=len(a['label']),metrics=values,expected_metrics=r['expected_metrics'],absolute_error=errors,
...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--manifest',type=Path,default=ROOT/'results/transformer_inputs.json');p.add_argument('--data-root',type=Path,required=True);p.add_argument('--output-dir',type=Path,required=True);p.add_argument('--worker',choices=['NEXT','SuperNEMO']);a=p.parse_args()
    if a.worker:return worker(a.worker,a.manifest,a.data_root,a.output_dir/(a.worker+'.json'))
    for ds in ['NEXT','SuperNEMO']:
        env=dict(os.environ,PYTHONDONTWRITEBYTECODE='1',MPLCONFIGDIR=str(a.output_dir/'matplotlib'))
        subprocess.run([sys.executable,'-B',__file__,'--manifest',str(a.manifest),'--data-root',str(a.data_root),'--output-dir',str(a.output_dir),'--worker',ds],env=env,check=True)
    print('All 12 published Transformer metric replays passed.')
