...
- Open [`energybench_workflow.ipynb`](energybench_workflow.ipynb) for the
  complete collaborator-facing tutorial.
...
Use Python 3.11. Install the PyTorch build appropriate for your CPU or CUDA
system, then install the remaining requirements:

```bash
python -m pip install -r requirements.txt
```

Run Jupyter from this directory so the local package is importable:

```bash
jupyter lab energybench_workflow.ipynb
```
