...
class ModelSpec:
    ...
    tokenization: SuperNEMOTokenizationConfig | None
...
def _checked_local_spec(identifier: str) -> ModelSpec:
    configured = ARCHITECTURES[identifier]
    if identifier in TRANSFORMER_ARCHITECTURE_IDS:
        model_class = SuperNEMOTransformerClassifier
    else:
        try:
            from architectures import get_model_class
        except ImportError as error:
            raise RuntimeError(
                "Wing's specialized architectures package is required only for "
                f"the specialized model {identifier!r}"
            ) from error
