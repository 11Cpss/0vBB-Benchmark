...
from supernemo_transformer import SuperNEMOTokenizationConfig, tokenize_tracker_event
...
class PreparedData:
    ...
    tokenization_config: SuperNEMOTokenizationConfig | None
    ...
    def __init__(
        ...
        *,
        ...
        if tokenization_config is not None:
            if not isinstance(
                tokenization_config, SuperNEMOTokenizationConfig
            ):
                raise TypeError(
                    "tokenization_config must be a SuperNEMOTokenizationConfig"
                )
...
def prepare_dataset(
    *,
    ...
    tokenization_config: SuperNEMOTokenizationConfig | None = None,
    batch_size: int,
    num_workers: int,
) -> PreparedData:
    ...
    if tokenization_config is not None:
        if not isinstance(tokenization_config, SuperNEMOTokenizationConfig):
            raise TypeError(
                "tokenization_config must be a SuperNEMOTokenizationConfig"
            )
        if input_kind != "sequence" or task != "classification":
    ...
    loaders = {}
    for split in SPLIT_NAMES:
        loader = DataLoader(datasets[split], **loader_options)
        # A multi-process DataLoader already prefetches batches in its worker
        # processes. Starting those processes from _PrefetchLoader's background
        # thread is both redundant and unsafe with fork/HDF5 on Linux. Retain
        # the small thread prefetcher only for the zero-worker fallback.
        loaders[split] = _PrefetchLoader(loader) if int(num_workers) == 0 else loader
