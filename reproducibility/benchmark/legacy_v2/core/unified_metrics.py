"""EnergyBench unified 5 keV plus overflow protocol; NumPy only, no model inference.

All masks are aligned to the original supplied test population. In particular
the matched reporting denominator precedes negative-energy exclusion.
"""
...
def evaluate(label, score, energy, group=None, weight=None, energy_unit='keV', config=None, return_arrays=False):
    ...
    result = {'protocol_version': config['protocol_version'], 'protocol_sha256': fingerprint(config),
              ...
              'n_zero_weight':int((w == 0).sum()), 'n_overflow':int((finite & (e>3000)).sum()), 'overflow_bin_index':600}
    def pop(mask):
    ...
    for name in sorted(np.unique(g)):
        ...
        counts = np.bincount(bins[eligible], minlength=601)
    ...
    # Matching support uses all admissible finite nonnegative energies, including overflow.
    common, retained = np.zeros(s.size,dtype=bool), np.zeros(s.size,dtype=bool)
    mw = np.zeros(s.size)
    counts, masses, target = np.zeros((2,601),dtype=int),np.zeros((2,601)),np.zeros(601)
    ...
    if all(np.any(energy_valid & (y == c)) for c in (0,1)):
        ...
        if hi < lo:
            ...
            for c in (0,1):
                sel = common & (y==c)
                counts[c] = np.bincount(bins[sel],minlength=601)
                masses[c] = np.bincount(bins[sel],weights=w[sel],minlength=601)
    ...
    for c in (0,1):
        ...
        stages = {'input':sel,'inclusive':sel&inc,'original_finite':sel&finite,
                  ...
                  'negative_energy':sel&finite&(e<0),'overflow':sel&finite&(e>3000),
