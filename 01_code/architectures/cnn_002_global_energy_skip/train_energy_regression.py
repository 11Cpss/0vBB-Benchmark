#!/usr/bin/env python3
"""
train_energy_regression.py — CNN-002 topology-only energy regression
...
MODEL_SUFFIX = os.environ.get(
    "NEXT_MODEL_SUFFIX",
    "_cnn_002_topology_only_energy_regression",
)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Train the CNN-002 topology-only NEXT energy regressor on CUDA"
        )
    )
    parser.add_argument(
        "--smoke",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print("Model     : CNN-002 topology-only energy regression%s" % model_suffix)
    print(
        "Input     : (3, %d, %d), %s"
        % (
