#!/usr/bin/env python3
"""
train_energy_regression_large.py — enlarged CNN-002 energy regression
...
MODEL_SUFFIX = os.environ.get(
    "NEXT_MODEL_SUFFIX",
    "_cnn_002_topology_only_energy_regression_large",
)
...
# The only model/training difference from train_energy_regression.py: widen the
# same two-convolution network from C=8 to C=64 (1,409 -> 75,777 parameters).
BASE_CHANNELS = _env_int("NEXT_BASE_CHANNELS", 64)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Train the enlarged CNN-002 topology-only NEXT energy regressor "
            "on CUDA"
        )
    )
    parser.add_argument(
...
def save_history_plot(
    ...
    axes[1].legend()
    figure.suptitle("CNN-002 energy regression%s" % model_suffix)
    figure.tight_layout()
...
def save_regression_plot(
    ...
    figure.suptitle("CNN-002 validation energy regression")
    figure.tight_layout()
...
def validate_checkpoint(
    ...
    if loaded.get("model_name") != "SimpleNextEnergyRegressor":
        raise ValueError(
            "checkpoint model is not CNN-002 topology-only regression: %s"
            % checkpoint_path
        )
    model_config = loaded.get("model_config")
    required_model_config = {"base_channels"}
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print(
        "Model     : enlarged CNN-002 topology-only energy regression%s"
        % model_suffix
    )
    print(
        "Input     : (3, %d, %d), %s"
    ...
    if model_configuration is None:
        model_configuration = {"base_channels": BASE_CHANNELS}
    if projection.representation != "binary_occupancy":
        raise ValueError("CNN-002 topology-only regression requires binary occupancy")
    ...
    # Record the untrained topology-only baseline before optimization, then let
    # later epochs replace it only when validation loss genuinely improves.
    ...
    best_validation_loss = float(initial_validation_metrics["loss"])
    print(
