#!/usr/bin/env python3
"""
train_classification.py — CNN-003 residual-spatial classifier

A deep residual model for 0nu-beta-beta versus Bi-214 classification. Each
event is converted to three fixed detector-coordinate energy projections (XY,
XZ and YZ). The model retains a coarse spatial grid and fuses explicit global
geometry instead of reducing all topology to one global average.
...
from next_cnn.model import ResidualSpatialNextCNN as NEXTCNN
...
MODEL_SUFFIX = os.environ.get(
    "NEXT_MODEL_SUFFIX",
    "_cnn_003_residual_spatial_classification",
)
...
BASE_CHANNELS = _env_int("NEXT_BASE_CHANNELS", 16)
POOLED_SIZE = _env_int("NEXT_POOLED_SIZE", 4)
HEAD_FEATURES = _env_int("NEXT_HEAD_FEATURES", 256)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train the CNN-003 NEXT binary classifier on CUDA"
    )
    parser.add_argument(
        "--smoke",
        action="store_true",
...
def save_history_plot(
    ...
    axes[1].legend()
    figure.suptitle("CNN-003 classification%s" % model_suffix)
    figure.tight_layout()
...
def validate_classification_checkpoint(
    ...
    if loaded.get("model_name") != "ResidualSpatialNextCNN":
        raise ValueError(
            "checkpoint model is not CNN-003 classification: %s"
            % checkpoint_path
        )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print("Model     : CNN-003 residual-spatial classification%s" % model_suffix)
    print(
    ...
    if not projection.normalize_energy:
        raise ValueError("CNN-003 classification requires normalized projections")
