#!/usr/bin/env python3
"""
train_classification.py — CNN-002 global-energy-skip classifier

A deliberately small model for 0nu-beta-beta versus Bi-214 classification.
Each event is converted to three fixed detector-coordinate energy projections
(XY, XZ and YZ). Two Conv2d layers learn topology while an FP32 global-sum
skip preserves the same paired architecture used for energy regression.
...
from next_cnn.model import GlobalEnergySkipCNN as NEXTCNN
...
MODEL_SUFFIX = os.environ.get(
    "NEXT_MODEL_SUFFIX",
    "_cnn_002_global_energy_skip_classification",
)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train the CNN-002 NEXT binary classifier on CUDA"
    )
    parser.add_argument(
        "--smoke",
        action="store_true",
...
def save_history_plot(
    ...
    axes[1].legend()
    figure.suptitle("CNN-002 classification%s" % model_suffix)
    figure.tight_layout()
...
def checkpoint_payload(
    ...
    return {
        ...
        "model_name": "GlobalEnergySkipCNN",
        ...
        "training_config": {
            "entrypoint": (
                "01_code/architectures/cnn_002_global_energy_skip/"
...
def validate_classification_checkpoint(
    ...
    if loaded.get("model_name") != "GlobalEnergySkipCNN":
        raise ValueError(
            "checkpoint model is not CNN-002 classification: %s"
            % checkpoint_path
        )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.full_validation:
        ...
        model_configuration = dict(loaded["model_config"])
    ...
    print("Model     : CNN-002 classification%s" % model_suffix)
    print(
    ...
    if not np.isclose(
        ...
        float(projection.input_scale),
    ):
        raise ValueError("model input_scale differs from projection input_scale")
    if not projection.normalize_energy:
        raise ValueError("CNN-002 classification requires normalized projections")
    if not np.isclose(
        float(model_configuration["global_center"]),
        1.0,
    ) or not np.isclose(
        float(model_configuration["global_scale"]),
        1.0,
    ):
        raise ValueError(
            "CNN-002 classification global normalization must be center=1, scale=1"
        )
    model = NEXTCNN(**model_configuration).to(device)
    model_channels = int(model_configuration["base_channels"])
    ...
    expected_parameter_count = (
        18 * model_channels * model_channels + 32 * model_channels + 4
    )
    if parameter_count != expected_parameter_count:
        raise RuntimeError(
            "CNN-002 parameter count changed: expected %d, found %d"
