...
def prepare_dataset(
    ...
    *,
    ...
    if manifest is None and cache_path.exists():
        raise ValueError(
            "Existing split manifest is incompatible with the source inventory "
            "or settings; refusing to regenerate or overwrite event assignments. "
            "Use a new manifest path only for a new experiment."
        )
    if manifest is None:
...
def _event_sample(
    ...
    *,
    ...
    image, coverage = project_event(coordinates, energies, projection)
    return {
        "inputs": image,
