"""Export per-event predictions after training. Final metrics use benchmark/evaluate.py."""
...
import numpy as np
import torch
from .training import _weighted_auc
...
def export_classification(model, dataloader, device="auto", output_dir="outputs/predictions", *, overwrite=False):
    """Save aligned logits, labels, physical energy and IDs; report ordinary AUC only."""
    destination = Path(output_dir).expanduser().resolve()
    path = destination / "predictions.npz"
    summary_path = destination / "prediction_summary.json"
    if not overwrite and (path.exists() or summary_path.exists()):
        raise FileExistsError(f"prediction outputs already exist in {destination}")
    arrays = _run_inference(model, dataloader, _resolve_device(device), True)
    # The NEXT loader supplies physical energy in MeV, with no range clipping.
    arrays["score"] = arrays["prediction"]
    arrays["energy_keV"] = arrays["energy"] * 1000.0
    arrays["group"] = arrays["category"]
    arrays["weight"] = arrays["sample_weight"]
    auc = _weighted_auc(arrays["label"], arrays["score"], arrays["sample_weight"])
    summary = {"task": "classification", "n_events": len(arrays["score"]),
               "auc": float(auc) if np.isfinite(auc) else None,
               "status": "predictions_exported", "energy_unit": "keV",
               "final_metrics": "benchmark/evaluate.py"}
