"""SuperNEMO checkpoint selection and native prediction export.

The matching helper is used on validation events only. Final test EnergyBench
metrics are calculated through the public benchmark entry points.
"""
...
def _profile_root() -> Path:
    for parent in Path(__file__).resolve().parents:
        candidate = parent / "benchmark/published_profiles/supernemo_transformer/energybench"
        if candidate.is_dir():
            if str(parent) not in sys.path:
                sys.path.insert(0, str(parent))
            return candidate
    raise ImportError("The publication benchmark profile is required beside code/.")

def energybench_code_sha256() -> str:
    """Hash the actual packaged selection implementation; never an old identity."""
    digest = hashlib.sha256()
    for path in sorted(_profile_root().glob("*.py")):
...
def matched_validation_auc(
    ...
    *,
    ...
    protocol = json.loads(EVALUATION_MANIFEST.read_text())
