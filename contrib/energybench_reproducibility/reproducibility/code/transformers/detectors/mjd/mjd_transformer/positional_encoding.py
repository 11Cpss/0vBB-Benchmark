"""Coordinate encodings for MJD waveform tokens."""
...
def validate_coordinates(coordinates: Tensor, coordinate_dim: int) -> None:
    """Validate ``[batch, tokens, coordinate_dim]`` coordinates."""
...
class CoordinateMLPEncoding(nn.Module):
    """Learn a nonlinear embedding of token coordinates."""
...
class FourierCoordinateEncoding(nn.Module):
    """Embed coordinates with learned projections of multiscale sinusoids."""
    ...
    def __init__(
        ...
        self.coordinate_dim = _positive_integer(coordinate_dim, "coordinate_dim")
        self.d_model = _positive_integer(d_model, "d_model")
        self.num_frequencies = _positive_integer(
...
def build_position_encoder(
    ...
    *,
    ...
    """Construct one positional encoder from its stable configuration name."""
    ...
    if name == "fourier_coordinates":
        return FourierCoordinateEncoding(
            coordinate_dim, d_model, num_frequencies
        )
    raise ValueError(
