...
% Preserve the template's Times family and real bold/italic under XeTeX.
% The regular pdfLaTeX/Overleaf path retains the original times package.
\ifdefined\XeTeXversion
  \usepackage{fontspec}
  \setmainfont{NimbusRoman}[Path=wing_contribution/fonts/,Extension=.otf,
    UprightFont=*-Regular,BoldFont=*-Bold,ItalicFont=*-Italic,BoldItalicFont=*-BoldItalic]
  \setsansfont{NimbusSans}[Path=wing_contribution/fonts/,Extension=.otf,
    UprightFont=*-Regular,BoldFont=*-Bold,ItalicFont=*-Italic,BoldItalicFont=*-BoldItalic]
  \setmonofont{NimbusMonoPS}[Path=wing_contribution/fonts/,Extension=.otf,
    UprightFont=*-Regular,BoldFont=*-Bold,ItalicFont=*-Italic,BoldItalicFont=*-BoldItalic]
\fi
...
\usepackage{hyperref}
\usepackage{url}
\input{wing_contribution/packages}


\title{Learning Physics, Not Energy: A Multimodal Benchmark for Neutrino Detectors}
...
\begin{abstract}
The experimental search for neutrinoless double-beta decay (NLDBD) would answer one of the most important questions in physics: Why is there more matter than antimatter in our universe? A key challenge in NLDBD searches is identifying NLDBD signal events from many unwanted background events. Deep learning classifiers have shown strong performance on this task, but they often exhibit energy shortcut learning: instead of learning event topology, which more directly reflects the underlying physics, they exploit event energy as an easier predictive feature. In this work, we introduce a comprehensive benchmark for quantitatively understand deep learning classifiers in NLDBD searches. Our benchmark makes three contributions: We coordinated the public release of four NLDBD datasets spanning diffierent modalities: time-series, point-cloud, and sparse 2D image. We then designed two metrics to quantatively understand topology-based classification power and the degree of energy shortcut learning. Based on datasets and metrics, we conducted a comprehensive benchmarking study over a set of modality-specfic deep learning models as well as a unified Transformer model across applied to all modalities. Our results provide insights into when strong classification performance reflects true physics learning, while establishing a pathway toward foundation models for NLDBD data.
\end{abstract}
...
\input{dataset_description/datasets_section}

% BEGIN Wing contribution: independently editable sections
\FloatBarrier
\input{wing_contribution/benchmarking_metrics}
\input{model_design}
\input{wing_contribution/experiment_results}
\input{wing_contribution/conclusion}
% END Wing contribution
...
\bibliography{iclr2027_conference,wing_contribution/references/wing_references,wing_contribution/references/models_refs,wing_contribution/references/supernemo_refs}
\bibliographystyle{iclr2027_conference}

\appendix
\FloatBarrier
\input{wing_contribution/appendices}
\input{transformer_contribution/sections/appendix_tokenization}
