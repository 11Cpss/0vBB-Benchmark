...
optional base sample weights. Only the published overlap target is supported.
"""
...
def evaluate_energy_matched_roc(
    ...
    *,
    ...
    target:
        ...
        Only the published overlap target is supported.
    n_bootstrap:
    ...
    if target_method != "overlap":
        raise ValueError("The published target must be overlap")
    if not 0.0 <= float(target_tpr) <= 1.0:
        raise ValueError("target_tpr must lie in [0, 1]")
