...
Provenance: everything below this docstring is a verbatim copy of
``supernemobench/tokenization.py`` from the published SuperNEMO Transformer
benchmark (branch ``wenyu/energybench-reproducibility``), which is not on
``main``.  It is duplicated so the RoPE classifier sees byte-identical token
content -- and therefore differs from the published coordinate-MLP and
Fourier-XYZ models *only* in how position is encoded.  Once that benchmark
lands on ``main``, replace this file with an import of it.
"""
